        ...
        {data?.map((rate, id) => (
          <div key={rate?.id} className="even:bg-pink-50 odd:bg-yellow-50 p-3 rounded-lg">
            <div className="flex justify-between">
              ...
              <div className="flex flex-col items-center text-end gap-1">
                <div className="flex items-center text-yellow-600">
                  {Array.from({ length: rate.rating }, (_, index) => (
                    <Star key={index} className="text-lg text-yellow-600 fill-current" />
                  ))}
                </div>
                <span className="w-full text-end">{rate.rating.toFixed(1)}</span>
              </div>
            </div>
          </div>
        ))}
